...
-- Segunda función de AntroLabsHD, independiente del radar comercial: guarda
-- discurso citable (voz directa, entrevistas, podcasts...) para lectura
-- humana, SIN scoring, SIN categoria comercial, SIN estado de candidato.
-- Nunca pasa por clasificacion_epistemologica.py ni promocion_candidatos.py
-- (autorizado por el operador —Mario—, 2026-09-18). `tipo_registro` solo
-- admite 'dato' o 'patron_observable': jamás 'inferencia' ni 'hipotesis',
-- eso es interpretación y no vive aquí.
...
CREATE TABLE IF NOT EXISTS fragmento_observado (
    id                  INTEGER PRIMARY KEY AUTOINCREMENT,
    fuente_id           INTEGER NOT NULL REFERENCES fuente_discursiva(id),
...
CREATE TABLE IF NOT EXISTS nota_de_mario (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    fragmento_id INTEGER NOT NULL REFERENCES fragmento_observado(id),
